...
if __name__ == "__main__":
    ...
    mpc_optimizer = MPCOptimizer(
        ...
        initial_parameters={
            'theveninBasedBattery.coulombSocCounter.SOC_init.k': 0.424669
        },
...
# final_time = (2*60+47)*60
    ...
    def cost_func(step_num: int, state: pd.DataFrame, input: pd.DataFrame, output: pd.DataFrame, run: neptune.Run) -> float:
        ...
        controlled_idx = state['time'].sub(time+3*step).abs().idxmin()

        controlled_timepoints = state['time'][:controlled_idx]
        ...
        # controlled_discharged_energy = np.maximum(input[:controlled_idx]['I_req'].values*capacity_ah*nominal_v, 0)*step
        controlled_discharged_energy = input[:controlled_idx]['I_req'].values*capacity_wh*step/3600/100
        # controlled_ref = get_ref_price(load_df, step, time, time+3*step)/100
        controlled_ref = np.array(list(map(partial(get_ref_price, expected_load, step), controlled_timepoints)))/100
        # controlled_horizon_cost = np.linalg.norm(get_per_mw_price(time) * (controlled_energy + controlled_discharged_energy) - get_ref_price(time, time+3*step))
        controlled_horizon_cost = np.linalg.norm((controlled_energy - controlled_discharged_energy) - controlled_ref)

        price = (controlled_energy - controlled_discharged_energy)

        run['cost/controlled_price'].log(np.mean(price))
        run['cost/controlled_ref'].log(np.mean(controlled_ref))

        predicted_timepoints = state['time'][controlled_idx:]
        ...
        predicted_horizon_cost = np.linalg.norm((predicted_energy - predicted_discharged) - predicted_ref)

        run['cost/predicted_energy'].log(np.mean(predicted_energy))
        run['cost/predicted_discharged'].log(np.mean(predicted_discharged))
        run['cost/predicted_price'].log(np.mean(predicted_energy-predicted_discharged))
    ...
    def visualize_output(step_num: int, states: pd.DataFrame, run: neptune.Run):
        ...
        for _, state in itertools.islice(states.iterrows(), len(states) - 1):
            ...
            # run['capacityFade'].log(state['theveninBasedBattery.capacityFadingCalc.capacityFade'])
    ...
    new_control_df = mpc_optimizer.optimize(
        ...
        constraints=[StateConstraint('SoC', 0.2, 0.8)]
    )
